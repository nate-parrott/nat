...
final class ThreadModelTests: XCTestCase {
    func testTruncateTaggedLLMessages() {
        ...
        let shortResult = messages[..<5].asArray.truncateTaggedLLMessages(keepFirstN: 2, keepLastN: 4)
        XCTAssertEqual(shortResult.count, 5, "Short arrays should not be truncated")
        
        // Test 2: Basic truncation with small values
        let result = messages.truncateTaggedLLMessages(keepFirstN: 2, keepLastN: 3, round: 4)
        ...
        let roundedResult = messages.truncateTaggedLLMessages(keepFirstN: 2, keepLastN: 3, round: 5)
        // Cutoff should be:
        ...
        let functionResult = messagesWithFunctions.truncateTaggedLLMessages(keepFirstN: 2, keepLastN: 3, round: 4)
